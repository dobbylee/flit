...
@MainActor
private final class RecordingStuckNotificationPlatform: StuckNotificationPlatform {
    struct AddedNotification: Equatable {
    ...
    var addAccepted: Bool
    var deliverOnAdd: Bool
    var delivered = Set<String>()
    private(set) var authorizationRequestCount = 0
    private(set) var deliveredReadCount = 0
    private(set) var added: [AddedNotification] = []

    init(
        authorization: NotificationAuthorizationState,
        authorizationGrant: Bool = false,
        addAccepted: Bool = true,
        deliverOnAdd: Bool = false
    ) {
        self.authorization = authorization
        self.authorizationGrant = authorizationGrant
        self.addAccepted = addAccepted
        self.deliverOnAdd = deliverOnAdd
    }
    ...
    func requestAuthorization(
        ...
        authorization = authorizationGrant ? .authorized : .denied
        completion(authorizationGrant)
    }

    func deliveredIdentifiers(
        completion: @escaping @MainActor @Sendable (Set<String>) -> Void
    ) {
        deliveredReadCount += 1
    ...
    func add(
        ...
        added.append(AddedNotification(identifier: identifier, title: title, body: body))
        if addAccepted, deliverOnAdd {
            delivered.insert(identifier)
        }
        completion(addAccepted)
    }
}
...
struct NativeHealthTests {
    static func main() throws {
        ...
        try require(
            StuckNotificationPresentationPolicy.foregroundOptions.contains(.banner)
                && !StuckNotificationPresentationPolicy.foregroundOptions.contains(.sound),
            "foreground notification policy must present the bounded card without inventing sound settings"
        )

        let dueNotificationFixture = try decodeFixture(
            FlitManagedStuckNotificationsDueReadResponse.self,
            at: "\(fixtureRoot)/managed_stuck_notifications_due_read.response.json"
        )
        let deliveredNotificationFixture = try decodeFixture(
            FlitManagedStuckNotificationDeliveredResponse.self,
            at: "\(fixtureRoot)/managed_stuck_notification_delivered.delivered.response.json"
        )
        let claimNotificationFixture = try decodeFixture(
            FlitManagedStuckNotificationDeliveryClaimResponse.self,
            at: "\(fixtureRoot)/managed_stuck_notification_delivery_claim.response.json"
        )
        let failedNotificationFixture = try decodeFixture(
        ...
        var notificationDueOutstanding = true
        var notificationReceiptRequests: [FlitManagedStuckNotificationDeliveredRequest] = []
        let notificationClient = StuckNotificationClient(
            dueLoader: { request in
                try require(
                    request.clientProtocolVersion == flitClientProtocolVersion,
                    "due read must submit only the generated protocol version"
                )
                return FlitManagedStuckNotificationsDueReadResponse(
                    protocolVersion: dueNotificationFixture.protocolVersion,
                    eventSchemaVersion: dueNotificationFixture.eventSchemaVersion,
                    notifications: notificationDueOutstanding ? [dueNotification] : []
                )
            },
            claimLoader: { request in
                try require(
                    request.runId == dueNotification.runId
                        && request.expectedRunVersion == dueNotification.runVersion
                        && request.occurrenceId == dueNotification.occurrenceId,
                    "delivery claim must preserve the exact due identity"
                )
                return claimNotificationFixture
            },
            failureLoader: { _ in failedNotificationFixture },
            receiptLoader: { request in
                notificationReceiptRequests.append(request)
                notificationDueOutstanding = false
                return deliveredNotificationFixture
            }
        )
        let deliveredPlatform = RecordingStuckNotificationPlatform(
            authorization: .authorized,
            deliverOnAdd: true
        )
        let deliveredCoordinator = StuckNotificationCoordinator(
            client: notificationClient,
            platform: deliveredPlatform
        )
        try deliveredCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            deliveredPlatform.added == [
                .init(
                    identifier: dueNotification.occurrenceId,
                    title: FoundationCopy.text(.notificationStuckTitle),
                    body: FoundationCopy.format(.notificationStuckBody, "Project Alpha")
                )
            ]
                && notificationReceiptRequests.count == 1
                && notificationReceiptRequests[0].runId == dueNotification.runId
                && notificationReceiptRequests[0].expectedRunVersion
                    == dueNotification.runVersion
                && notificationReceiptRequests[0].occurrenceId
                    == dueNotification.occurrenceId
                && notificationReceiptRequests[0].platformId
                    == dueNotification.occurrenceId,
            "authorized notification must record the exact receipt only after delivered-list confirmation"
        )
        let renderedNotification = deliveredPlatform.added[0]
        try require(
            !renderedNotification.title.contains(dueNotification.runId)
                && !renderedNotification.body.contains(dueNotification.runId)
                && !renderedNotification.body.contains(dueNotification.occurrenceId)
                && renderedNotification.body == "Project Alpha · Possibly stuck",
            "notification copy must contain only bounded project and category facts"
        )
        try deliveredCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            deliveredPlatform.added.count == 1 && notificationReceiptRequests.count == 1,
            "a durable receipt must remove the due item without duplicate native delivery"
        )

        var pendingReceiptCount = 0
        var pendingClaimed = false
        var pendingFailureCount = 0
        let pendingClient = StuckNotificationClient(
            dueLoader: { _ in
                FlitManagedStuckNotificationsDueReadResponse(
                    protocolVersion: dueNotificationFixture.protocolVersion,
                    eventSchemaVersion: dueNotificationFixture.eventSchemaVersion,
                    notifications: [
                        FlitManagedStuckNotificationDueRecord(
                            runId: dueNotification.runId,
                            runVersion: dueNotification.runVersion,
                            occurrenceId: dueNotification.occurrenceId,
                            platformId: dueNotification.platformId,
                            deliveryClaimed: pendingClaimed
                        )
                    ]
                )
            },
            claimLoader: { _ in
                let alreadyClaimed = pendingClaimed
                pendingClaimed = true
                return FlitManagedStuckNotificationDeliveryClaimResponse(
                    protocolVersion: claimNotificationFixture.protocolVersion,
                    runId: claimNotificationFixture.runId,
                    runVersion: claimNotificationFixture.runVersion,
                    occurrenceId: claimNotificationFixture.occurrenceId,
                    platformId: claimNotificationFixture.platformId,
            ...
            failureLoader: { _ in
                ...
                return failedNotificationFixture
            },
            receiptLoader: { _ in
        ...
        let pendingPlatform = RecordingStuckNotificationPlatform(
            authorization: .authorized,
            addAccepted: true,
            deliverOnAdd: false
        )
        let pendingCoordinator = StuckNotificationCoordinator(
            client: pendingClient,
            platform: pendingPlatform
        )
        try pendingCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try pendingCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        ...
        try require(
            pendingPlatform.added.count == 1
                && pendingReceiptCount == 0
                && pendingClaimed,
            "accepted scheduling without delivered-list evidence must retain one durable claim without duplicate or success"
        )
        pendingPlatform.delivered.insert(dueNotification.occurrenceId)
        try pendingCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            pendingPlatform.added.count == 1 && pendingReceiptCount == 1,
            "a later exact delivered-list observation must reconcile the pending receipt"
        )

        let deniedPlatform = RecordingStuckNotificationPlatform(authorization: .denied)
        let deniedCoordinator = StuckNotificationCoordinator(
            client: pendingClient,
            platform: deniedPlatform
        )
        try deniedCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            deniedPlatform.authorizationRequestCount == 0
                && deniedPlatform.added.isEmpty
                && pendingReceiptCount == 1,
            "denied authorization must keep Core due state without prompting, scheduling, or receipt"
        )

        let firstAuthorizationPlatform = RecordingStuckNotificationPlatform(
            authorization: .notDetermined,
            authorizationGrant: true,
            deliverOnAdd: true
        )
        pendingClaimed = false
        let firstAuthorizationCoordinator = StuckNotificationCoordinator(
            client: pendingClient,
            platform: firstAuthorizationPlatform
        )
        try firstAuthorizationCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            firstAuthorizationPlatform.authorizationRequestCount == 1
                && firstAuthorizationPlatform.added.count == 1
                && pendingReceiptCount == 2,
            "not-determined authorization must request once and still require exact delivery evidence"
        )

        let restartPlatform = RecordingStuckNotificationPlatform(authorization: .authorized)
        pendingClaimed = true
        restartPlatform.delivered.insert(dueNotification.occurrenceId)
        let restartCoordinator = StuckNotificationCoordinator(
            client: pendingClient,
            platform: restartPlatform
        )
        try restartCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            restartPlatform.added.isEmpty && pendingReceiptCount == 3,
            "restart reconciliation must recover an already delivered platform ID without rescheduling"
        )

        let dismissedRestartPlatform = RecordingStuckNotificationPlatform(
            authorization: .authorized
        )
        pendingClaimed = true
        let dismissedRestartCoordinator = StuckNotificationCoordinator(
            client: pendingClient,
            platform: dismissedRestartPlatform
        )
        try dismissedRestartCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            dismissedRestartPlatform.added.isEmpty
                && pendingReceiptCount == 3
                && pendingClaimed,
            "restart after a dismissed delivered notification must preserve the durable claim without a second alert or false receipt"
        )

        let failedPlatform = RecordingStuckNotificationPlatform(
            authorization: .authorized,
            addAccepted: false
        )
        pendingClaimed = false
        let failedCoordinator = StuckNotificationCoordinator(
            client: pendingClient,
            platform: failedPlatform
        )
        try failedCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try failedCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        ...
        try require(
            failedPlatform.added.count == 2
                && pendingFailureCount == 2
                && !pendingClaimed,
            "an explicit scheduling failure must release the durable claim and remain retryable"
        )

        var transientClaimed = false
        var transientFailureCount = 0
        let transientFailureClient = StuckNotificationClient(
            dueLoader: { _ in
                FlitManagedStuckNotificationsDueReadResponse(
                    protocolVersion: dueNotificationFixture.protocolVersion,
                    eventSchemaVersion: dueNotificationFixture.eventSchemaVersion,
                    notifications: [
                        FlitManagedStuckNotificationDueRecord(
                            ...
                            occurrenceId: dueNotification.occurrenceId,
                            platformId: dueNotification.platformId,
                            deliveryClaimed: transientClaimed
                        )
                    ]
                )
            },
            ...
            failureLoader: { _ in
                ...
                if transientFailureCount == 1 {
                    throw NativeHealthTestFailure.failed("transient delivery release failure")
                }
            ...
            receiptLoader: { _ in
                throw NativeHealthTestFailure.failed(
                    "an unconfirmed retry must not write a delivery receipt"
                )
            }
        )
        let transientFailurePlatform = RecordingStuckNotificationPlatform(
            authorization: .authorized,
            addAccepted: false
        )
        let transientFailureCoordinator = StuckNotificationCoordinator(
            client: transientFailureClient,
            platform: transientFailurePlatform
        )
        try transientFailureCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            transientFailurePlatform.added.count == 1
                && transientFailureCount == 1
                && transientClaimed,
            "a transient release failure must retain the exact failed release for retry"
        )
        transientFailurePlatform.addAccepted = true
        try transientFailureCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            transientFailurePlatform.added.count == 2
                && transientFailureCount == 2
                && transientClaimed,
            "the next reconciliation must release the failed claim and schedule one retry"
        )
        try transientFailureCoordinator.reconcile(
            projectNamesByRunId: [dueNotification.runId: "Project Alpha"]
        )
        try require(
            transientFailurePlatform.added.count == 2,
            "an accepted unconfirmed retry must retain its new claim without duplicate scheduling"
        )
