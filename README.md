...
- The current build does not yet provide the planned Core-owned PTY and terminal surface. Phase 5 found that public `libghostty-vt` requires a Flit-owned renderer, while exact stable xterm failed the mandatory Korean IME gate. V1 accepts retained-view-only continuity within one live app process. Exact SwiftTerm `v1.18.0` now passes the physical Korean 2-Set composition, Caps Lock, and Escape gate with every key bound to its macOS input-source identity; remaining interaction, accessibility, security, resource, and Core-owned lifecycle gates are still required before any production dependency or implementation.
