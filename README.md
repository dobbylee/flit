...
- The current build does not yet provide the planned Core-owned PTY and terminal surface. Phase 5 found that public `libghostty-vt` requires a Flit-owned renderer. The exact stable xterm stack passed current AppKit/WKWebView resource, deterministic-correctness, and bounded control-isolation checks, but an attested raw-event run failed the mandatory Korean IME gate, so xterm is not the production candidate. Native SwiftTerm is the next feasibility candidate; host-owned I/O, IME, accessibility, presentation continuity, remaining security/privacy, resource, and Core-owned lifecycle gates still precede any production dependency.
