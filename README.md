...
- The current build does not yet provide the planned Core-owned PTY and terminal surface. Phase 5 found that public `libghostty-vt` requires a Flit-owned renderer, while exact stable xterm failed the mandatory Korean IME gate. Exact SwiftTerm `v1.15.0` passed the static host-owned I/O boundary but has no public lossless presentation-state restore contract. V1 now accepts retained-view-only continuity within one live app process, so SwiftTerm interaction, accessibility, security, resource, and lifecycle feasibility may continue; no terminal candidate is approved for production dependency or implementation until those gates pass.
