...
- The current build does not yet provide the planned Core-owned PTY and terminal surface. Phase 5 found that public `libghostty-vt` requires a Flit-owned renderer, while exact stable xterm failed the mandatory Korean IME gate. V1 accepts retained-view-only continuity within one live app process, but exact SwiftTerm `v1.15.0` then emitted compatibility-jamo code points `U+314E U+314F U+3134` before `U+AE00 abc`, instead of composed `U+D55C U+AE00 abc`, at its physical Korean IME host-input boundary. The candidate stopped at that first hard failure; no terminal candidate is approved for production dependency or implementation.
